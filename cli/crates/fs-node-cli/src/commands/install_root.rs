...
pub async fn show() -> Result<()> {
    ...
    println!("FreeSynergy install paths ({})", InstallPaths::config_file_path().display());
    println!();
    println!("  system   {}", paths.system_base.display());
    println!("           Apps:              {}/apps/<name>/", paths.system_base.display());
    println!("           Bots:              {}/bots/<name>/", paths.system_base.display());
    println!("           Adapters:          {}/adapters/<name>/", paths.system_base.display());
    println!();
    println!("  config   {}", paths.config_base.display());
    println!("           Widgets:           {}/widgets/<name>/", paths.config_base.display());
    println!("           Tasks:             {}/tasks/<name>.toml", paths.config_base.display());
    println!("           i18n:              {}/i18n/<locale>/", paths.config_base.display());
    println!("           Themes/Styles/…:   {}/themes|styles|…/<name>.toml", paths.config_base.display());
    println!();
    println!("  font     {}", paths.font_base.display());
    println!("           Font sets:         {}/<name>/", paths.font_base.display());
    println!();
    println!("  icon     {}", paths.icon_base.display());
    println!("           Icon sets:         {}/<name>/", paths.icon_base.display());
    println!();
    println!("  cursor   {}", paths.cursor_base.display());
    println!("           Cursor sets:       {}/<name>/", paths.cursor_base.display());
    println!();
...
pub async fn set(base: &str, new_path: PathBuf) -> Result<()> {
    ...
    println!("  fsn config install-root migrate {base} {}", new_path.display());
    Ok(())
}
...
pub async fn migrate(base: &str, new_path: PathBuf) -> Result<()> {
    ...
    let affected: &[ResourceType] = match base {
        ...
        _        => bail!("Unknown base '{}'. Valid: system, config, font, icon, cursor", base),
    };
...
async fn get_installed_names() -> Vec<(String, ResourceType)> {
    let Some(conn) = crate::db::get_conn() else { return Vec::new() };
